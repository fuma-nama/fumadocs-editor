...
- **`@fumadocs-editor/core`**: parse, incremental serialize, component specs,
  dialects (math, directive admonitions, files fences, heading suffixes).
- **`@fumadocs-editor/sync`**: FS mirror, CAS autosave, block-level merge,
  Yjs collab, server auth with per-doc scopes.
